...
public final class FileCommandsLoader implements CommandsLoader {
    ...
    private static final int COMMAND_LENGTH_SIZE = 36;
    ...
    private String transformLine(@NonNull String inputLine, int counter) {
        ...
        for (byte b : bytes) {
            if (b == ' ') continue;
            if (b != 'x' && b != '0' && b != '1')
                throw new IllegalArgumentException(String.format("Illegal symbol: %c on line: %d", b, counter));
            if (b == '1') builder.append("1");
            else builder.append("0");
            commandLengthCounter++;
        }
        if (commandLengthCounter != COMMAND_LENGTH_SIZE)
            throw new IllegalArgumentException("Command length must be" + COMMAND_LENGTH_SIZE + ", " + inputLine);
